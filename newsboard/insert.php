  ...
  @extract($_SESSION);
	...
	for ($i=0; $i<$count; $i++) {
		...
		if (!$upfile_error[$i]) {
            ...
            // move_uploaded_file(임시저장파일명,실제저장할 서버경로 파일명 ) 
			{
				echo("
					<script>
	...
	if ($mode=="modify") {
		...
		$sql = "update $table set subject='$subject', content='$content', is_html='$is_html' where num=$num";
		mysql_query($sql, $connect);  
	}	else	{
		if ($html_ok=="y")	{
			$is_html = "y";
	...
	echo "
		<script>
			location.href = 'list.php?table=$table&page=$page&scale=$scale&show=$show#content';
		</script>
	";
?>